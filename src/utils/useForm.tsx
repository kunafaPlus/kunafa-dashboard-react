...
const useForm = (options: UseFormOptions = { mode: 'onChange' }) => {
  ...
  const validateField = useCallback((name: string, value: any): boolean => {
    ...
    let error = '';
    if (rules.required) {
      const isEmpty = value === undefined || value === null || (typeof value === 'string' && value.trim() === '') || (typeof value === 'number' && isNaN(value));
    ...
    if (!error && rules.minLength !== undefined && value.length < rules.minLength) {
      ...
      for (const [validationName, validationFn] of Object.entries(rules.validate)) {
  ...
  const register = useCallback((name: string, rules: ValidationRules = {}, defaultValue: any = '') => {
  ...
  const handleSubmit = useCallback((callback: (data: FormData) => void) => (e: React.FormEvent) => {
    e.preventDefault();
    let isValid = true;

    const allFields = Object.keys(validationRulesRef.current);
    allFields.forEach((name) => {
      const value = formData[name] !== undefined ? formData[name] : validationRulesRef.current[name]?.defaultValue;
