...
const SortButton = React.forwardRef<HTMLButtonElement, SortButtonProps>(
  (
    ...
    const [direction, setDirection] = React.useState<'asc' | 'desc'>(defaultDirection);
    ...
    return (
      <div className="relative inline-block">
        <button
          ref={buttonRef}
          type="button"
          className={cn(sortButtonVariants({ variant, size }), 'gap-2', className)}
          ...
          >
            {options.map((option) => (
              <button
                ...
                className={cn(
                  'relative flex w-full cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors',
                  {
                    ...
                    'bg-accent text-accent-foreground': option.id === selectedOption,
              ...
              >
                ...
                {option.label}
                {option.id === selectedOption && <span className="ml-auto">{directionIcon}</span>}
              </button>
            ))}
          </div>
        )}
