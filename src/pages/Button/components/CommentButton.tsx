...
interface CommentButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement>,
    VariantProps<typeof commentButtonVariants> {
  count?: number;
  showCount?: boolean;
  showIcon?: boolean;
  onClick?: () => void;
  ...
  (
    ...
    return (
      <button
        ref={ref}
        type="button"
        className={cn(commentButtonVariants({ variant, size }), 'gap-2', className)}
        onClick={handleClick}
        disabled={loading}
        {...props}
      >
        ...
        {buttonText}
        {showCount && count > 0 && <span className="text-muted-foreground">{count}</span>}
      </button>
    );
  }
);
