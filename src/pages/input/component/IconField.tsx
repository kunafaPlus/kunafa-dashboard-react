...
const iconFieldVariants = cva('relative inline-flex items-center w-full group', {
  variants: {
    variant: {
  ...
  (
      ...
      return (
        <div
          className={cn(
            ...
            iconClickable && 'cursor-pointer hover:text-primary active:scale-95',
    ...
    return (
      <div className="w-full">
        <div className={cn(iconFieldVariants({ variant, size, iconPosition }), containerClassName)}>
        ...
        {(error || hint) && (
          <div
            ...
            id={error ? `${props.id}-error` : hint ? `${props.id}-hint` : undefined}
          >
            {error || hint}
          </div>
        )}
